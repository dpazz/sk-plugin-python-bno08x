...
[Note] Depending on the wiring of the breakout the default I2C address may be 0x4A [74] or 0x4B [75]. The plugin scans I2C to find the actual address (if any) and compares it with the one defined in parameters schema logging a warning if the address found is different from the one defined. Absence of either adresses stops the plugin
